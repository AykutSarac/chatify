...
const getUsers = async (req, reply) => {
    ...
    try {
        ...
        const data = await instance.get('/channels/' + channelId).then(res => res.data?.users.map(id => 'id=' + id))
        const userList = await instance.get(`/users?${data.join('&')}`).then(res => res.data?.sort(sortByName))
